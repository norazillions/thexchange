...
const PinVerification = () => {
  ...
  const processTransfer = async (userData) => {
    ...
    try {
      ...
      if (isInternalTransfer && receiverId && receiverData) {
        console.log('📝 === CREATING RECEIVER TRANSACTION ===');
        ...
        await updateDoc(receiverRef, {
          totalBalance: receiverCurrentBalance + recipientAmount,
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-[480px] mx-auto px-4 sm:px-6 py-4 sm:py-6"
    >
      <div className="relative flex items-center justify-center h-12 mb-6">
        <button
          onClick={() => navigate(-1)}
        ...
        <h1 className="text-base md:text-lg font-semibold text-[#111111]">Enter PIN</h1>
      </div>
      ...
      {isSuccess ? (
        ...
        <>
          ...
          <div className="flex justify-center gap-3 mb-6" onPaste={handlePaste}>
            {[0, 1, 2, 3].map((index) => (
              <input  key={index}  ref={inputRefs[index]}  type="password"  maxLength="1"  value={pin[index]}
                ...
                autoFocus={index === 0}  inputMode="numeric"  pattern="[0-9]*"  disabled={isLoading}/>
            ))}
          </div>
